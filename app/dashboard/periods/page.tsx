...
export default function PeriodsPage() {
  ...
  useEffect(() => {
    ...
    listAccountingPeriodsForCurrentOrganization()
      .then((result) => {
        if (!active) return;
        if (!result.ok) {
          setError("Die Buchungszeiträume konnten nicht geladen werden.");
          return;
        }
      ...
      setPeriods((current) =>
        current.map((period) =>
          period.id === target.id
            ? {
                ...period,
                status: target.status === "open" ? "locked" : "open",
              }
            : period,
        ),
      );
  ...
  return (
    <section className={styles.page} aria-busy={loading}>
      ...
      <article className={styles.summaryPanel}>
        <header className={styles.header}>
          ...
          <span className={styles.adminBadge}>
            <Lock aria-hidden="true" /> Nur für Administratoren
          </span>
        </header>
        <div className={styles.summaryStats}>
          <div className={styles.summaryStat}>
            ...
            <span>
              <small>Aktiver Zeitraum</small>
              <strong>{activePeriod ? periodLabel(activePeriod) : "Keiner"}</strong>
            </span>
          </div>
          <div className={styles.summaryStat}>
            <span className={`${styles.summaryIcon} ${styles.summaryIconGreen}`}>
          ...
          <div className={styles.summaryStat}>
            ...
            <span>
              <small>Gesperrte Zeiträume</small>
      ...
      <article className={styles.listPanel}>
        ...
        <div className={styles.rows}>
          {!loading && !periods.length ? (
            <p className={styles.empty}>Keine Buchungszeiträume gefunden.</p>
          ) : null}
          {periods.map((period) => (
            <div className={styles.row} key={period.id}>
              <strong>{periodLabel(period)}</strong>
              <span
                className={`${styles.status} ${period.status === "open" ? styles.open : styles.locked}`}
              >
                {period.status === "open" ? (
                  <Check aria-hidden="true" />
                ) : (
              ...
              <button
                ...
                onClick={() => openPeriodAction(period)}
              >
                {period.status === "open" ? (
                  <Lock aria-hidden="true" />
                ) : (
        ...
        <div className={styles.infoNote}>
          ...
          <span>
            Gesperrte Zeiträume schützen abgeschlossene Buchungen vor unbeabsichtigten Änderungen.
          </span>
        </div>
      </article>
      ...
      {target ? (
        <Dialog
          label={target.status === "open" ? "Buchungszeitraum sperren" : "Buchungszeitraum entsperren"}
          onClose={() => {
        ...
        >
          <header className={styles.dialogHeader}>
            <div>
              <h2>
                {target.status === "open"
                  ? "Buchungszeitraum sperren?"
                  : "Buchungszeitraum entsperren?"}
              </h2>
              <p>{periodLabel(target)}</p>
            </div>
            <button
              type="button"
          ...
          <div className={styles.dialogBody}>
            <span className={styles.dialogIcon}>
              {target.status === "open" ? <Lock /> : <Unlock />}
            </span>
            <p>
              {target.status === "open"
                ? "Gebuchte Transaktionen können nach dem Sperren nicht mehr geändert werden."
                : "Der Zeitraum wird wieder für berechtigte Änderungen geöffnet."}
            </p>
            <label>
              <span>Grund</span>
              <textarea
                ...
                placeholder="z. B. Monatsabschluss geprüft …"
                rows={3}
              />
            </label>
            {message ? (
