...
} from "@/features/finance/actions/wallets";
...
export default function PeriodsPage() {
  ...
  useEffect(() => {
    ...
    listAccountingPeriodsForCurrentOrganization()
      ...
      .catch(() => {
        if (active) setError("The accounting periods could not be loaded.");
      })
      .finally(() => {
  ...
  return (
    <section className={styles.page} aria-busy={loading}>
      <LoadingStatus
        loading={loading}
        label="Accounting periods are loading…"
      />
      {error ? (
      ...
      <article className={styles.summaryPanel}>
        <header className={styles.header}>
          <div>
            ...
            <p>
              Close reviewed months so posted transactions remain traceable.
            </p>
          </div>
          <span className={styles.adminBadge}>
            <Lock aria-hidden="true" /> Admins only
          </span>
        </header>
      ...
      <article className={styles.listPanel}>
        ...
        <div className={styles.rows}>
          {!loading && !periods.length ? (
            <p className={styles.empty}>No Accounting periods gefunden.</p>
          ) : null}
          {periods.map((period) => (
            <div className={styles.row} key={period.id}>
              ...
              >
                ...
                {period.lockedByName ? (
                  <small>by {period.lockedByName}</small>
                ) : null}
              </span>
              <button
                type="button"
        ...
        <div className={styles.infoNote}>
          ...
          <span>
            Locked periods protect completed bookings from accidental changes.
          </span>
        </div>
      </article>
      ...
      {target ? (
        <Dialog
          label={
            target.status === "open"
              ? "Accounting period sperren"
              : "Accounting period entsperren"
          }
          onClose={() => {
        ...
        >
          ...
          <div className={styles.dialogBody}>
            ...
            <p>
              {target.status === "open"
                ? "Posted transactions cannot be changed after locking."
                : "The period will be reopened for authorized changes."}
            </p>
            <label>
              <span>Reason</span>
              <textarea
                ...
                placeholder="e.g. month-end close checked …"
                rows={3}
                autoFocus
              />
            </label>
