...
function IconBubble({
  ...
  return (
    <span
      className={`flex size-8 shrink-0 items-center justify-center rounded-none !bg-transparent text-ink ${tone} ${className}`}
    >
      {children}
    </span>
  );
          ...
          {dashboardTransactions.map((transaction, index) => {
            ...
            return (
              ...
              >
                ...
                <span className="pl-10 text-xs text-muted-foreground sm:pl-0 sm:text-sm">
                  {transaction.category}
                </span>
                <span className="ui-tabular pl-10 text-xs text-muted-foreground sm:pl-0 sm:text-sm">
                  {transaction.date}
                </span>
                <span
                  className={`whitespace-nowrap pl-10 font-medium tabular-nums sm:pl-0 ${transaction.amount.startsWith("+") ? "text-[var(--ui-positive)]" : "text-[var(--ui-negative)]"}`}
                >
                  {transaction.amount}
                </span>
        ...
        >
          {dashboardGoals.slice(0, 3).map((goal) => (
            ...
            >
              ...
              <p
                className={`${dashboardStyles.goalSaved} mt-3 text-sm text-muted-foreground`}
              >
                {goal.saved}
              </p>
      ...
      <CardContent className="space-y-4 px-5 pb-5 pt-4 sm:px-6 lg:px-7 min-[1280px]:max-[2199px]:space-y-2 min-[1280px]:max-[2199px]:pt-2 min-[2200px]:space-y-1 min-[2200px]:pb-6 min-[2200px]:pt-1">
        {dashboardCategories.map((category) => {
          ...
          return (
            <div
              key={category.title}
              className="grid min-h-12 grid-cols-[36px_minmax(90px,120px)_minmax(0,1fr)_40px] items-center gap-3 text-sm min-[1280px]:max-[2199px]:min-h-10 min-[2200px]:min-h-20 min-[2200px]:grid-cols-[40px_minmax(110px,140px)_minmax(0,1fr)_44px] min-[2200px]:gap-4 min-[2200px]:text-base"
            >
              <IconBubble
                tone={category.bubble}
                className="min-[2200px]:size-10"
              >
                <Icon className="size-4 min-[2200px]:size-5" />
              </IconBubble>
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground min-[2200px]:mt-1.5 min-[2200px]:text-sm">
                  {category.amount}
                </p>
              </div>
              <span className="text-right text-sm text-muted-foreground min-[2200px]:text-base">
                {category.progress}%
              </span>
            </div>
          );
