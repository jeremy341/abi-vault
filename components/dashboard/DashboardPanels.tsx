...
export function GoalsPanel() {
  ...
  function addGoal() {
    setGoalItems((current) => current.length > goals.length ? current : [
      ...current,
      { title: "Klassenfahrt", target: "2.500 €", saved: "750 € gesammelt", progress: 30, date: "15.08.2026" },
    ]);
  }
  ...
  return (
    <Card className="rounded-2xl bg-card py-0 shadow-sm">
      ...
      <CardContent className={`${hasAdditionalGoal ? "overflow-x-auto" : "overflow-visible"} px-4 pb-4 pt-4 sm:px-5 lg:px-6 lg:pb-5`}>
        <div className={`grid gap-3 ${hasAdditionalGoal ? "min-w-[660px] grid-cols-3" : "min-w-0 grid-cols-3"}`}>
          {goalItems.map((goal) => (
            <div key={goal.title} className="rounded-xl border border-black/10 p-4 dark:border-white/15">
              ...
              <p className="mt-3 text-sm text-muted-foreground">{goal.saved}</p>
              <div className="mt-4 flex items-center gap-3"><div className="h-2 flex-1 overflow-hidden rounded-full bg-black/[0.08] dark:bg-white/[0.12]"><div className="h-full rounded-full bg-ink dark:bg-white" style={{ width: `${goal.progress}%` }} /></div><span className="text-sm font-medium text-muted-foreground">{goal.progress}%</span></div>
              <p className="mt-5 text-sm text-muted-foreground">Ziel: {goal.date}</p>
            </div>
          ))}
        </div>
      </CardContent>
      ...
      <CardContent className="space-y-4 px-5 pb-5 pt-4 sm:px-6 lg:px-7 min-[1280px]:max-[2199px]:space-y-2 min-[1280px]:max-[2199px]:pt-2 min-[2200px]:space-y-1 min-[2200px]:pb-6 min-[2200px]:pt-1">
        {categories.map((category) => {
          ...
          return <div key={category.title} className="grid min-h-12 grid-cols-[36px_minmax(90px,120px)_minmax(0,1fr)_40px] items-center gap-3 text-sm min-[1280px]:max-[2199px]:min-h-10 min-[2200px]:min-h-20 min-[2200px]:grid-cols-[40px_minmax(110px,140px)_minmax(0,1fr)_44px] min-[2200px]:gap-4 min-[2200px]:text-base">
            ...
            <div>
              <div className="h-2 overflow-hidden rounded-full bg-black/[0.07] dark:bg-white/[0.12] min-[2200px]:h-3"><div className={`h-full rounded-full ${category.color}`} style={{ width: `${category.progress}%` }} /></div>
