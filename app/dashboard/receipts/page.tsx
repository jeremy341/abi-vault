...
const transactionOptions = [
  ...
  { value: "Abizeitung Druckkosten", label: "Abizeitung Druckkosten", date: "12.05.2024", amount: "-320,00 €" },
  { value: "Kuchenverkauf", label: "Kuchenverkauf", date: "11.05.2024", amount: "+185,50 €" },
  { value: "Dekoration Abiball", label: "Dekoration Abiball", date: "08.05.2024", amount: "-184,90 €" },
  { value: "Spende Eltern", label: "Spende Eltern", date: "07.05.2024", amount: "+250,00 €" },
  { value: "Abifahrt Bus", label: "Abifahrt Bus", date: "05.05.2024", amount: "-1.200,00 €" },
  { value: "T-Shirts Abijahrgang", label: "T-Shirts Abijahrgang", date: "03.05.2024", amount: "-950,00 €" },
] as const;
...
function TransactionCombobox({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  const filteredOptions = transactionOptions.filter((option) =>
    `${option.label} ${option.date} ${option.amount}`.toLowerCase().includes(search.trim().toLowerCase()),
  );
    ...
    <label className={styles.formField}>
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          ...
          <div className={styles.comboboxList} role="listbox" aria-label="Verfügbare Transaktionen">
            {filteredOptions.length ? filteredOptions.map((option) => (
              ...
              >
                <span>
                  ...
                  {option.date ? <small>{option.date}</small> : <small>Beleg später zuordnen</small>}
                </span>
                {option.amount ? (
                  <b className={option.amount.startsWith("+") ? styles.positive : styles.negative}>{option.amount}</b>
                ) : null}
                {option.value === value ? <Check className={styles.comboboxCheck} aria-hidden="true" /> : null}
              </button>
            )) : <p className={styles.comboboxEmpty}>Keine passende Transaktion gefunden.</p>}
          </div>
        </PopoverContent>
      </Popover>
    </label>
...
export default function ReceiptsPage() {
  ...
  const pageSize = useResponsivePageSize({ defaultSize: 9, landscapeSize: 6, wideSize: 10 });
  ...
  return <section className={styles.page}>
    ...
    <article className={styles.listCard}>
      ...
      <div className={`${styles.tableWrap} ui-data-table`}>
        ...
        <div className={styles.rows}>{visible.map((receipt) => <div className={styles.row} key={receipt.id}>
          <span className={styles.fileCell}><span className={styles.fileIcon}><FileText /></span><span><strong>{receipt.file}</strong></span></span>
          <span data-label="Transaktion" className={styles.transactionCell}><strong>{receipt.transaction}</strong></span>
          <span data-label="Datum" className={`ui-tabular ${styles.muted}`}>{receipt.date}</span><span data-label="Betrag" className={`ui-tabular ${receipt.amount < 0 ? styles.negative : styles.positive}`}>{formatAmount(receipt.amount)}</span>
        <span data-label="Status" className={`ui-badge ${styles.statusTag} ${receipt.status === "Geprüft" ? styles.checked : receipt.status === "Zu prüfen" ? styles.review : styles.unassigned}`}>{receipt.status === "Geprüft" ? <Check /> : receipt.status === "Zu prüfen" ? <Clock3 /> : <Link2 />}{receipt.status}</span>
          <button type="button" className={styles.moreButton} aria-label={`${receipt.file} Optionen`}><MoreVertical /></button>
        </div>)}</div>
      </div>
      <footer className={styles.pagination}><span>{filtered.length ? `${(currentPage - 1) * pageSize + 1}–${Math.min(currentPage * pageSize, filtered.length)} von ${filtered.length}` : "0 von 0"}</span><Pagination page={currentPage} pageCount={pageCount} onPageChange={setPage} className={styles.pageButtons} /></footer>
    </article>
    ...
    {modalOpen ? <Dialog label="Beleg hinzufügen" onClose={closeModal} overlayClassName={styles.overlay} dialogClassName={styles.modal}>
      ...
      <TransactionCombobox value={transaction} onChange={setTransaction} />
      </div>
