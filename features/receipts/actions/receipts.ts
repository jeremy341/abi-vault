...
export async function uploadReceipt(
  ...
  if (file.size > 5 * 1024 * 1024) return invalidReceipt("Der Receipt darf höchstens 5 MB groß sein.");
...
export async function updateReceiptMetadata(
  ...
  if (error) {
    ...
    if (error.code === "23503") return actionFailure("NOT_FOUND", "The receipt is no longer available.");
    if (error.code === "55000") return actionFailure("PERIOD_LOCKED", "This receipt cannot be changed for this transaction.");
...
export async function createReceiptDownloadUrl(
  ...
  const { data: receipt, error } = await supabase
    ...
    .is("archived_at", null)
    .maybeSingle();
  if (error || !receipt) return actionFailure("NOT_FOUND", "Der Receipt was nicht gefunden.");
