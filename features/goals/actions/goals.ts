...
function mapGoalError(code?: string) {
  if (code === "42501") return actionFailure("FORBIDDEN", "You are not allowed to manage goals.");
  if (code === "55000") return actionFailure("CONFLICT", "The goal or period is not available.");
  if (code === "23514" || code === "22023") return actionFailure("INVALID_PAYLOAD", "The goal data is invalid.");
  if (code === "23505") return actionFailure("CONFLICT", "This goal request was already submitted.");
