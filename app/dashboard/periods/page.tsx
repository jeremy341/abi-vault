    ...
    <section className={styles.page} aria-busy={loading}>
      ...
      <article className={styles.summaryPanel}>
        ...
        <div className={styles.summaryStats}>
          ...
          <div className={styles.summaryStat}><span className={`${styles.summaryIcon} ${styles.summaryIconGreen}`}><Check aria-hidden="true" /></span><span><small>Offene Periods</small><strong>{openCount}</strong></span></div>
          <div className={styles.summaryStat}><span className={styles.summaryIcon}><Lock aria-hidden="true" /></span><span><small>Gesperrte Periods</small><strong>{lockedCount}</strong></span></div>
        </div>
      </article>
      <article className={styles.listPanel}>
        <header className={styles.listHeader}><h2>2026</h2></header>
        <div className={styles.tableHeader}><span>Zeitraum</span><span>Status</span><span>Action</span></div>
      ...
      {target ? (
        <Dialog label={target.status === "open" ? "Buchungszeitraum sperren" : "Buchungszeitraum entsperren"} onClose={() => { if (!saving) setTarget(null); }} overlayClassName={styles.overlay} dialogClassName={styles.dialog}>
          ...
          <div className={styles.dialogBody}>
            ...
            <label><span>Grund</span><textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="e.g. month-end close reviewed …" rows={3} autoFocus /></label>
